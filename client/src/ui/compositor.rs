...
impl Compositor {
    ...
    pub async fn from(settings: Settings) -> Option<Self> {
...
impl iced_graphics::window::Compositor for Compositor {
    ...
    fn new(settings: Self::Settings) -> Result<(Self, Renderer), Error> {
        let compositor =
            futures::executor::block_on(Self::request(settings)).ok_or(Error::AdapterNotFound)?;
    ...
    fn create_surface<W: HasRawWindowHandle>(&mut self, window: &W) -> wgpu::Surface {
        #[allow(unsafe_code)]
        unsafe {
