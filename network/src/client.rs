...
impl<S: Socket> Client<S> {
    pub fn new(socket: S, server_addr: SocketAddr) -> Self {
        Self {
            ...
            status: Status::Disconnected {
                message: "Not yet connected".to_owned(),
            },
    ...
    pub fn connect(&mut self) {
        match &self.status {
            Status::Disconnected { .. } => {
                ...
                self.status = Status::ConnectSent {
                    client_salt,
                    time: Instant::now(),
                };
            }
            ...
            if let Ok(packet) = deserialize_packet(&mut self.buf[0..packet_size]) {
                match &mut self.status {
                    Status::ConnectSent { client_salt, .. } => {
                        ...
                        match packet {
                            ToClientPacket::Challenge {
                                client_salt: packet_client_salt,
                                server_salt,
                            } => {
                    ...
                    | Status::Connected { salts_xor, .. } => {
                        // Did we receive a normal message ?
                        match packet {
                            ToClientPacket::Message {
                                salts_xor: message_salts_xor,
                                messages,
                            } => {
                                if *salts_xor == message_salts_xor {
                                    ...
                                    } = &mut self.status
                                    {
                                        for msg in messages {
                                            match msg {
                                                Message::Unreliable(data) => self
                                                    .messages
                                                    .push((MessageDelivery::Unreliable, data)),
    ...
    pub fn tick(&mut self) {
        ...
        match &mut self.status {
            Status::ConnectSent { client_salt, time } => {
                // Timeout
                ...
                serialize_packet(&mut self.buf, &connect_packet)
                    .expect("Failed to serialize TryConnect packet");
                self.socket.send(&mut self.buf, self.server_addr);
            }
            Status::ChallengeResponseSent { salts_xor, time } => {
                // Timeout
                if Instant::now() - *time > DISCONNECT_TIMEOUT {
                    self.status = Status::Disconnected {
                        message: TIMEOUT_MESSAGE.to_owned(),
                ...
                let connect_packet = ToServerPacket::ChallengeResponse {
                    salts_xor: *salts_xor,
                    padding: Default::default(),
                };
                serialize_packet(&mut self.buf, &connect_packet)
                    .expect("Failed to serialize ChallengeResponse packet");
                self.socket.send(&mut self.buf, self.server_addr);
            }
            Status::Connected {
                last_server_packet,
                ...
                let mut send_message = |message| {
                    ...
                    if serialize_packet(buf, &packet).is_err() {
                        ...
                        serialize_packet(buf, &packet)
                            .expect("Failed to serialize packet to server");
