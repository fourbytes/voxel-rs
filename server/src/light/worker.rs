use super::sunlight::{compute_light, FastBFSQueue};
use super::HighestOpaqueBlock;
use std::sync::Arc;
use voxel_rs_common::{
    collections::zero_initialized_vec,
    worker::{Worker, WorkerState},
    world::{Chunk, LightChunk, CHUNK_SIZE},
};
...
impl ChunkLightingState {
    pub(self) fn new() -> Self {
        Self {
            ...
            light_data_reuse: unsafe {
                zero_initialized_vec((CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE * 27) as usize)
            },
            opaque_reuse: unsafe {
                zero_initialized_vec((CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE * 27) as usize)
            },
...
impl WorkerState<ChunkLightingData, Arc<LightChunk>> for ChunkLightingState {
    fn compute(&mut self, data: ChunkLightingData) -> Arc<LightChunk> {
        let pos = data.chunks[9 + 3 + 1]
            .as_ref()
            .expect("No middle chunk")
            .pos;
        Arc::new(LightChunk {
            light: compute_light(
                data.chunks,
