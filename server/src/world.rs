...
impl World {
    ...
    pub fn get_block(&self, pos: BlockPos) -> BlockId {
        match self.chunks.get(&pos.containing_chunk_pos()) {
            ...
            Some(server_chunk) => server_chunk
                .chunk
                .get_block_at(pos.pos_in_containing_chunk()),
        }
    }
    ...
    pub fn set_chunk(&mut self, chunk: Arc<Chunk>) {
        ...
        let chunk_column =
            self.chunk_columns
                .entry(pos.into())
                .or_insert_with(|| ServerChunkColumn {
        ...
        for pos in player_close_chunks {
            if let Some(server_chunk) = self.chunks.get(&pos) {
                if server_chunk.needs_light_update && !server_chunk.is_in_light_queue {
                    let res = self
                        .light_worker
                        .enqueue(self.create_chunk_lighting_data(*pos));
                    match res {
    ...
    pub fn send_chunks_to_player(
        &mut self,
        player_chunk: ChunkPos,
        data: &mut super::PlayerData,
    ) -> Vec<(Arc<Chunk>, Arc<LightChunk>)> {
            ...
            if let Some(server_chunk) = self.chunks.get(&pos) {
                ...
                if let Some(old_client_version) = loaded {
                    if old_client_version < server_chunk.version {
                        updates
                            .push((server_chunk.chunk.clone(), server_chunk.light_chunk.clone()));
                    }
                } else {
